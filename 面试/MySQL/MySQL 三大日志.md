...
## 如果一个事务产生的 redo log 超过 redo log 文件大小怎么办？

InnoDB 的 redo log 是循环使用的，并不是一个事务对应一个固定空间。当事务产生大量 redo 时，会不断追加 redo record。如果单条记录的redo log 太大, 还会拆分 redo log record.

如果 redo log 空间不足，InnoDB 会通过 checkpoint 将已经持久化的数据页刷盘，并释放对应 redo 空间继续写入。但如果存在超大事务，导致大量未提交修改无法 checkpoint 释放，最终可能造成 redo log 空间不足，事务阻塞。因此生产环境通常避免超大事务，并通过批量提交控制事务规模。
...
InnoDB 刷脏页主要有以下几类原因：
1. **后台线程定期刷新（正常情况）**
2. **redo log 空间不足触发 checkpoint 刷盘**
3. **Buffer Pool 脏页比例过高**
4. **MySQL 正常关闭**
5. **LRU 淘汰页时发现脏页**
6. **刷新压力调节（adaptive flushing）**
7. **DDL 或表空间相关操作触发**
# binlog
...
- `sync_binlog` 为 N (N >= 2)时, 每次事务提交时进行 write, 当累积 N 个事务提交后, 进行 fsync, 是一种折中的方案. 在出现 IO 瓶颈场景中, 将 N 设为一个较大值可以提升性能.
